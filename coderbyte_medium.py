def recursiveSol(number, count, string):
    # base condition
    if count > 10:
        return count
    ...
    for i in str(number):
        product = number * int(i)
        currentString = f'{number}{number * int(i)}'
        possibleAnswers.append(recursiveSol(product, count+1, currentString))
